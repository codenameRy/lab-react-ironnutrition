...
class App extends Component {
  ...
  //lab https://github.com/codenameRy/lab-react-ironnutrition
  displayFoods = () => {
...
//When we click on + sign, we add a state to the div
//Use state to add li

quantityChangeHandler = (e, index) => {
    console.log(e.target.value, index);
    let foodItem = {...this.state.allFoods[index]};
...
//Iteration 3 Add new food
  addFood = (e) => {
    ...
    updatedFoods.unshift(newFoodCopy);
...
// Iteration 4 Implement search bar
  search = (e) => {
    let foodsArray = foods.filter((eachFood) => {
      return eachFood.name.toLowerCase().includes(e.target.value); //make it lower case
    });
    this.setState({ allFoods: foodsArray });
  };

  //Iteration 3 Display form
  displayForm = () => {
    if (this.state.showForm) {
      return (
        <form onSubmit={this.addFood}>
          ...
          />
          <button><h2>Add new food</h2></button>
        </form>
      );
    } else {
  ...
  //Iteration 4 Search Bar Continued
  showForm = () => {
    this.setState({
